...
.. Перечитать оригинал, текст как-то не совсем понятен и последователен
...
.. rubric:: Применение и прицеливание
.. ~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. Вернуться потом к этому разделу, тут явно потребуется некоторое переписывание. Но сейчас просто опечатки и прям явные ошибки правлю
...
.. Почему "The increases to Difficulty are additive" перевели как "С этим следует увеличение Сложности"?
...
| Дальность: Касание
| Длительность: Сцена
| Если цель будет затронута магическим эффектом во время действия этого заклинания, она не будет поражена. После этого действие Опеки завершается.
...
За дополнительную 1 Душу получаемая скорость или количество игнорируемых провоцированных атак могут быть увеличены на 2.
...
| Дальность A: Касание
| Дальность Б: Близко
| Телепортирует цель в пределах дальности A в клетку, которую заклинатель может ощутить в пределах дальности Б от цели.
...
.. "Где-то в пределах досягаемости" это в оригинале "somewhere else within range", т.е.я так понимаю range это дальность Касание (первая). Но как лучше это написать пока фз
...
.. Перечитать оригинал, а то например вопрос да/нет можно задать цели и без ее знания об этом, а тут не указано
